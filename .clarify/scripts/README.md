...
| --- | --- | --- |
| `status` | `workflows/status.md` | Artifact inventory, Document Profile, outstanding `A/Q/S/V` (draft ids minus decision-log), archived final versions. |
| `answers <file\|->` | `workflows/improve.md` (mode `answers`) | Parse a filled Answer Sheet → normalized decisions JSON; **append** non-keep/skip decisions to `decision-log.md` (creates it with header if absent; never rewrites old rows). Applying decisions to the draft remains the LLM's job. |
