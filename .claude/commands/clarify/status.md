...
2. Reply in chat with: artifact inventory (exists/missing + producing command),
   the Document Profile (role, `Standard: URD`, domain, language), unresolved
   `A/Q/S/V` counts + top blockers, and the recommended next command on the pipeline
   `from-idea → improve answers → finalize → export`.
